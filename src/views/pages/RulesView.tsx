...
type FormState = { name: string; description: string; condition: string; status: 'A' | 'I' };
const emptyForm: FormState = { name: "", description: "", condition: "", status: "A" };
...
export function RulesView() {
    ...
    const handleRowClick = (rule: typeof rules[0]) => {
        if (selectedId === rule.id) {
            ...
            setEditMode(false);
            setForm({ name: rule.name, description: rule.description, condition: rule.condition, status: rule.status });
        }
    };

    const handleEditToggle = () => {
        if (editMode && selectedRule)
            setForm({ name: selectedRule.name, description: selectedRule.description, condition: selectedRule.condition, status: selectedRule.status });
        setEditMode(e => !e);
    };
    ...
    const handleSave = async () => {
        ...
        setIsSaving(true);
        if (!selectedId) {
            const payload: CreateRule = { name: form.name, description: form.description, condition: form.condition, status: "A" };
        ...
        <div className="flex flex-row flex-1 min-h-0 w-full h-full overflow-hidden border border-border-default">
            ...
            <div className="flex flex-col bg-[#FBFBFB] shrink-0 p-6 gap-2" style={{ width: "54%" }}>
                ...
                <div className="grid grid-cols-[0.5fr_1.5fr_2fr_0.8fr] px-1 py-1 border-b border-border-default shrink-0">
                    {["ID", "RULE NAME", "CONDITION", "STATUS"].map(h => (
                        <div key={h} className="text-xs font-mono font-bold tracking-label text-txt-secondary uppercase text-center">{h}</div>
                    ))}
                </div>
                        ...
                        {pageItems.map(rule => {
                            ...
                            return (
                                <div
                                    ...
                                    className={`grid grid-cols-[0.5fr_1.5fr_2fr_0.8fr] px-1 py-1.5 cursor-pointer select-none text-center border-b border-border-subtle transition-colors ${isSelected ? "bg-bg-selected border-l-2 border-l-accent" : "hover:bg-bg-tertiary border-l-2 border-l-transparent"}`}
                                >
                                    <div className="font-mono text-xs font-bold text-accent flex items-center justify-center">{String(rule.id).padStart(3, "0")}</div>
                                    <div className="font-mono text-xs text-txt-primary uppercase flex items-center justify-center truncate px-1">{rule.name}</div>
                                    <div className="font-mono text-xs text-status-info flex items-center justify-center truncate px-1">{rule.condition}</div>
                                    <div className="flex items-center justify-center">
                                        <span className={`font-mono text-xs font-bold uppercase ${statusInfo.className}`}>{statusInfo.label}</span>
                                    </div>
                                </div>
                            );
                        })}
                        {pageItems.length === 0 && (
                            <div className="py-10 text-center font-mono text-xs text-txt-disabled uppercase tracking-label">No rules found.</div>
                        )}
                    </div>
                )}

                <div className="bg-bg-secondary border-t border-border-default px-4 py-1.5 shrink-0">
                    <span className="font-mono text-xs text-txt-secondary tracking-wide uppercase">FOUND: {String(filtered.length).padStart(4, "0")}</span>
                </div>

                <div className="flex gap-2 h-10 shrink-0">
                    <button onClick={() => setPage(p => Math.max(0, p - 1))} disabled={page === 0}
                        className="flex-1 bg-bg-tertiary border border-border-default text-txt-primary font-mono font-bold text-sm tracking-wide uppercase hover:bg-bg-selected hover:border-accent transition-colors disabled:opacity-40 cursor-pointer disabled:cursor-default rounded-none">PREV</button>
                    <button onClick={() => setPage(p => Math.min(totalPages - 1, p + 1))} disabled={page >= totalPages - 1}
                        className="flex-1 bg-accent text-accent-fg font-mono font-bold text-sm tracking-wide uppercase hover:bg-accent-hover transition-colors disabled:opacity-40 cursor-pointer disabled:cursor-default rounded-none">NEXT</button>
                </div>
            </div>
            ...
            <div className="flex flex-col flex-1 bg-bg-secondary border-l border-border-default relative min-h-0">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        {selectedId && (
                            <button onClick={handleEditToggle}
                                className={`px-4 py-1.5 font-mono text-xs font-bold uppercase tracking-label rounded-none transition-colors cursor-pointer ${editMode ? "bg-bg-tertiary text-txt-primary border border-border-default" : "bg-accent text-accent-fg hover:bg-accent-hover"}`}>
                ...
                <div className="flex-1 px-8 pt-8 pb-4 flex flex-col gap-3 overflow-y-auto">
                    {selectedId && (
                        <TextFieldFloat label="ID" value={String(selectedId).padStart(3, "0")} readOnly />
                    )}
                    <TextFieldFloat label="RULE NAME" value={form.name} readOnly={!!selectedId && !editMode}
                        onChange={(e) => setForm(p => ({ ...p, name: e.target.value }))} />
                    <TextFieldFloat label="DESCRIPTION" value={form.description} readOnly={!!selectedId && !editMode}
                        onChange={(e) => setForm(p => ({ ...p, description: e.target.value }))} />
                    <TextFieldFloat label="CONDITION" value={form.condition} readOnly={!!selectedId && !editMode}
                ...
                <div className="px-8 py-6 shrink-0 flex flex-col gap-2">
                    {!selectedId && (
                        <button onClick={() => { void handleSave(); }}
                            disabled={isSaving || !form.name || !form.description || !form.condition}
                    ...
                    <button onClick={() => { setSelectedId(null); setEditMode(false); setForm(emptyForm); }}
