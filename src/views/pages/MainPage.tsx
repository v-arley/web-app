...
import { Boxes, LayoutDashboard, Users, Send, Database, Tent, Filter } from "lucide-react";
import {
...
export function DashboardPage() {
    ...
    const { activeKey, activeSection, sections, navigate: navTo } = useDashboardNav(availableSections);
    ...
    useEffect(() => {
        ...
        campSvc.findAll().then(res => {
            if (res.getEstado()) {
                const camps = res.getResultado<Camp[]>("registros") ?? [];
                setActiveCamp(camps.find(c => c.state === "A" || c.active) ?? camps[0] ?? null);
            }
        }).catch(() => { /* ignore */ });
    }, [authContext.campId]);
            ...
            <header className="bg-bg-secondary border-b border-border-default flex justify-between items-end px-8 py-3 shrink-0">
                <div className="flex flex-col gap-0.5">
                    <div className="flex items-center gap-2 text-[11px] text-txt-disabled font-mono tracking-label uppercase">
                        ...
                        {activeCamp && activeCamp.location_x != null && activeCamp.location_y != null && (
                            <>
                                <span className="text-border-default mx-0.5">|</span>
                                <span>COORDS: {activeCamp.location_x.toFixed(3)}, {activeCamp.location_y.toFixed(3)}</span>
                            </>
                        )}
                    </div>
                    <div className="font-mono tracking-[0.2em] uppercase leading-tight">
                        <span className="text-[25px] text-txt-secondary font-normal">CAMP </span>
                        <span className="text-[25px] text-txt-primary font-bold">{activeCamp?.code ?? "ALPHA"}</span>
                    </div>
                </div>
            </header>
            ...
            <footer className="w-full bg-bg-secondary border-t border-border-default flex flex-col sm:flex-row justify-between items-center px-8 py-3">
                <div className="flex flex-col sm:flex-row items-center gap-6 sm:gap-10 text-[11px] text-txt-disabled font-mono tracking-label uppercase">
                    <div className="flex items-center gap-5">
                        <span>Time: <span className="text-txt-secondary">{formatTime(currentDate)}</span></span>
                        <span>Date: <span className="text-txt-secondary">{formatDate(currentDate)}</span></span>
                    </div>
                ...
                <button className="mt-3 sm:mt-0 text-[11px] font-mono uppercase text-txt-disabled hover:text-accent transition-colors tracking-label" onClick={handleLogout}>
                    [ Log Out ]
                </button>
            </footer>
        </div>
