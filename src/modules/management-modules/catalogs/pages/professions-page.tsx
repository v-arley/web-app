...
import { useProfessionCatalogTab } from "../hooks/useProfessionCatalogTab";
...
function ProfessionsPageContent() {
    ...
    const professions = useProfessionCatalogTab(true, callbacks);
    const resources = useResourceCatalogTab(true, callbacks);

    const resourceOptions = useMemo(
        () => resources.allRecords.map((r) => ({ id: r.id ?? 0, label: `${r.code} / ${r.name}` })),
        [resources.allRecords],
    );
    ...
    return (
        <div className="mm-scope flex h-full flex-col overflow-hidden">
            ...
            {professions.error ? (
                <div className="px-5 py-2">
                    <AlertBanner tone="error" message={professions.error.message} />
                </div>
            ) : null}
            {resources.error ? (
                <div className="px-5 py-2">
            ...
            <div className="flex min-h-0 flex-1 flex-col lg:flex-row overflow-hidden">
                <div className="flex flex-1 flex-col border-b lg:border-b-0 lg:border-r border-border-default overflow-hidden">
                    ...
                    <div className="flex items-center gap-2 px-5 py-3 border-b border-border-default bg-bg-primary/30">
                        <div className="relative flex-1">
                            ...
                            <input
                                type="search"
                                placeholder="BUSCAR POR CODIGO, NOMBRE O RECURSO..."
                                ...
                                onChange={(event) => professions.setSearch(event.target.value)}
                            />
                        </div>
                        <div className="flex h-10 items-center whitespace-nowrap text-[10px] font-mono font-bold text-accent uppercase tracking-widest border-l border-border-default pl-4 ml-2">
                            REGISTROS: {String(professions.filteredRecords.length).padStart(4, "0")}
                        </div>
                    </div>
                    ...
                    {professions.isLoading ? (
                        ...
                        <ProfessionTable
                            professions={professions.filteredRecords}
                            selectedId={professions.selected?.id ?? null}
                            onSelect={professions.selectById}
                        />
                    )}
                </div>
                ...
                <aside className="flex w-full flex-col lg:w-[400px] xl:w-[450px] shrink-0 bg-bg-primary/20 overflow-hidden">
                    <ProfessionForm
                        key={professions.selected?.id ?? "new-profession"}
                        initialData={professions.selected}
                        resourceOptions={resourceOptions}
                        onSave={professions.handleSave}
                        onDelete={professions.handleDelete}
                        onClear={professions.handleClear}
                    />
                </aside>
            </div>
        </div>
