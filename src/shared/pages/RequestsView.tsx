          ...
          <div className="w-full px-8 py-5 flex justify-between gap-5 z-10 sticky top-0">
            <div className="flex items-center gap-2">
              <button
                ...
                className={`flex items-center gap-2 px-5 py-2 text-[11px] font-bold uppercase font-mono tracking-widest transition-all ${
                  activeTab === "received"
              ...
              >
                ...
                Received
                <span
                  className={`ml-1 px-1.5 py-0.5 rounded text-[9px] leading-none ${
                    activeTab === "received"
            ...
            <div className="relative w-80">
              <Search
                className="absolute left-3 top-1/2 transform -translate-y-1/2 text-txt-disabled"
                size={16}
              />
              <input
                type="text"
            ...
            {filteredRequests.length === 0 ? (
              ...
              <div className="flex flex-col gap-1">
                {filteredRequests.map((request) => (
                  <div
                    key={request.id}
                    className="w-full bg-bg-primary border border-border-default px-4 py-2 flex flex-col md:flex-row md:items-center justify-between gap-6 cursor-pointer transition-all group hover:translate-x-2"
                  >
                    <div className="flex flex-col">
                      <div className="flex items-center gap-3 text-[10px] font-mono text-txt-disabled tracking-widest uppercase">
                        ...
                        />
                        <span>
                          {request.requester === "Current User"
                            ? "CURRENT BASE"
                            : "BASE-CENTRAL"}
                        </span>
                        ...
                        <MapPin
                          ...
                          className="group-hover:text-accent transition-colors"
                        />
                        <span>{request.targetCamp.split(" - ")[0]}</span>
                      </div>
            ...
            <form onSubmit={handleNewRequest} className="flex flex-col gap-4">
              <div className="flex flex-col gap-1">
                ...
                <div className="relative">
                  <select
                    value={targetCamp}
                    onChange={(event) => setTargetCamp(event.target.value)}
                  ...
                  <div className="absolute right-4 top-1/2 transform -translate-y-1/2 pointer-events-none p-1 bg-bg-selected">
                    <svg
                      className="w-3 h-3 text-txt-secondary"
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
